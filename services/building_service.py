...
class BuildingService:
    ...
    @staticmethod
    def get_campus_options() -> dict:
        """获取校区选项"""
        return CAMPUS_OPTIONS.copy()

    @staticmethod
    def get_campus_name(campus_code: str) -> str:
        """获取校区名称"""
        return CAMPUS_OPTIONS.get(campus_code, campus_code)

    @staticmethod
    def get_buildings(campus: str) -> List[Building]:
        """获取校区楼栋列表"""
        if campus == "OldS":
            return OLD_CAMPUS_BUILDINGS.copy()
        return NEW_CAMPUS_BUILDINGS.copy()

    @staticmethod
    def get_building_by_index(campus: str, index: int) -> Optional[Building]:
        """根据索引获取楼栋"""
        buildings = BuildingService.get_buildings(campus)
        if 1 <= index <= len(buildings):
            return buildings[index - 1]
        return None

    @staticmethod
    def parse_campus_input(text: str) -> Optional[str]:
        """解析校区输入"""
        text = text.strip()
    ...
    def format_building_menu(campus: str, page: int = 1, page_size: int = 20) -> str:
        """格式化楼栋选择菜单"""
        buildings = BuildingService.get_buildings(campus)
        campus_name = BuildingService.get_campus_name(campus)
    ...
    @staticmethod
    def get_total_pages(campus: str, page_size: int = 20) -> int:
        """获取总页数"""
        buildings = BuildingService.get_buildings(campus)
        return (len(buildings) + page_size - 1) // page_size
